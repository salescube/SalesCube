...
<table id="search_result" summary="社員検索結果" class="forms detail_info" style="table-layout: auto; margin-top: 20px;">
	...
	<tr>
		...
		<td style="text-align: center">
			<c:if test="${isUpdate}">
			<button class="btn_small"  onclick="editUser('${sw:u(bean.userId)}');">編集</button>
			<button class="btn_small"  onclick="deleteUser('${sw:u(bean.userId)}', '${bean.updDatetm}');">削除</button>
			</c:if>
			<c:if test="${!isUpdate}">
			<button class="btn_small"  onclick="editUser('${sw:u(bean.userId)}');">参照</button>
			</c:if>
		</td>
	</tr>
	</c:forEach>
