...
<table id="slip_section" summary="slipSection" class="forms" style="table-layout: fixed;">
	<tr>
		<c:forEach var="colInfo" items="${slipColList}" varStatus="status">
			<th class=xl64><bean:message key='${colInfo.propKey}'/></th>
		</c:forEach>
	</tr>
	<c:forEach var="rowData" items="${slipList}" varStatus="statusRow">
		<tr>
			<c:forEach var="colInfo" items="${slipColList}" varStatus="statusCol">
				<c:choose>
