...
<!-- UNIT部 -->
<BR>
<table id="unit_section" summary="unitSection" class="forms" style="table-layout: fixed;">
	<tr>
		<c:forEach var="colInfo" items="${unitColList}" varStatus="status">
			<th class=xl64><bean:message key='${colInfo.propKey}'/></th>
		</c:forEach>
	</tr>
	<c:forEach var="rowData" items="${unitList}" varStatus="statusRow">
		<tr>
			<c:forEach var="colInfo" items="${unitColList}" varStatus="statusCol">
				<c:choose>
				<c:when test="${rowData[colInfo.name] == null}">
					<td class=xl70 style="text-align: left">
						&nbsp
