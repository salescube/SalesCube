<div style="width: 910px; height: 25px;">
		...
		<table id="search_result" summary="検索結果" class="forms" style="width: 910px">
			...
			<tr>
				<th style="cursor: pointer" rowspan="2" onclick="sort('rankCode');">顧客ランク<br>コード
    			<c:if test="${sortColumn == 'rankCode'}">
    				<c:if test="${sortOrderAsc}">▲</c:if>
    				<c:if test="${!sortOrderAsc}">▼</c:if>
    			</c:if>
                </th>
				<th style="cursor: pointer" rowspan="2"onclick="sort('rankName');">顧客ランク名
    			<c:if test="${sortColumn == 'rankName'}">
    				<c:if test="${sortOrderAsc}">▲</c:if>
    				<c:if test="${!sortOrderAsc}">▼</c:if>
    			</c:if>
                </th>
				<th style="cursor: pointer" rowspan="2"onclick="sort('rankRate');">値引率
	...
	<c:forEach var="bean" items="${searchResultList}" varStatus="status">
			<tr>
        		...
        		<td style="text-align: center">
        			<c:if test="${isUpdate}">
        			<button onclick="editCustomerRank('${bean.rankCode}');">編集</button>
        			<button onclick="deleteCustomerRank('${bean.rankCode}', '${bean.updDatetm}');">削除</button>
        			</c:if>
        			<c:if test="${!isUpdate}">
        			<button onclick="editCustomerRank('${bean.rankCode}');">参照</button>
        			</c:if>
        		</td>
			</tr>
	</c:forEach>
