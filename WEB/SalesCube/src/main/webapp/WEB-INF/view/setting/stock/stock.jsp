...
<body>
	...
	<div id="main_function">
		...
		<div class="function_buttons">
			...
			</button><button tabindex="2011" disabled="disabled">F12<br>&nbsp;
			</button>
		</div>
		...
		<s:form action="update">
			...
			<div class="function_forms">
				...
				<table id="scompany_info1" class="forms" style="width: 700px" summary="在庫管理情報">
					...
					<tr>
						<th><bean:message key="labels.stockHoldTermCalcCategory"/></th>
						<td>過去
							<html:select property="stockHoldTermCalcCategory"  tabindex="100">
					...
					<tr>
						<th><bean:message key="labels.deficiencyRate"/><bean:message key="labels.must"/></th>
						<td><html:text style="width: 50px; ime-mode: disabled; text-align: right;" tabindex="101" property="deficiencyRate" maxlength="4"/></td>
					</tr>
					<tr>
						<th><bean:message key="labels.maxEntrustPoTimelag"/><bean:message key="labels.must"/></th>
				...
				<table id="scompany_info1" class="forms" style="width: 700px" summary="在庫管理情報">
					...
					<tr>
						<th><bean:message key="labels.stockHoldDays"/><bean:message key="labels.must"/></th>
					...
					<tr>
						<th><bean:message key="labels.minPoLotCalcDays"/><bean:message key="labels.must"/></th>
						...
						<th><bean:message key="labels.minPoLotNum"/><bean:message key="labels.must"/></th>
						<td><html:text style="width: 50px; ime-mode: disabled; text-align: right;" tabindex="203" property="minPoLotNum" maxlength="3"/></td>
					</tr>
					<tr>
						<th><bean:message key="labels.maxPoNumCalcDays"/><bean:message key="labels.must"/></th>
					...
					<tr>
						<th><bean:message key="labels.minPoNum"/><bean:message key="labels.must"/></th>
				...
				<div style="text-align: right; width: 700px">
					<button tabindex="250" onclick="onF1()" style="width: 80px;" >リセット</button>
					<c:if test="${isUpdate == true}">
					<button tabindex="251" onclick="onF3()" style="width: 80px;" >更新</button>
					</c:if>
					<c:if test="${isUpdate == false}">
					<button tabindex="251" onclick="onF3()" style="width: 80px;" disabled="disabled">更新</button>
					</c:if>
				</div>
			</div>
		</s:form>
