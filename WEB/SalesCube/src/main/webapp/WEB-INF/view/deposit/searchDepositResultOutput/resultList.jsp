<table id="search_result" summary="searchResult" class="forms detail_info" style="table-layout: auto;">
	<colgroup>
		...
		<tr>
			<c:forEach var="value" items="${rowData}" varStatus="statusCol">
					<c:choose>
						...
						<c:when test="${isInputValid && columnInfoList[statusCol.index].itemId == 'depositSlipId'}">
							<c:if test='${outputExcel}'>
								<td class=xl70 style="text-align: ${columnInfoList[statusCol.index].textAlign}">&nbsp;${f:h(value)}&nbsp;</td>
							</c:if>
							<c:if test='${!outputExcel}'>
								<c:forTokens var="slipId" items="${value}" delims=" - " varStatus="statusCell">
									<bean:define id="concatUrl" value="${'/deposit/inputDeposit/load/?depositSlipId='}${slipId}" />
